...
from app.models import Contact, PhoneNumber, CallLog, GroupContactLink, Message, CallRun
...
class DialerService:
    """Service for initiating voice calls."""
    ...
    async def start_call_run(
        ...
        """
        Create a call run and dial multiple contacts.
        ...
        try:
            ...
            # Store message temporarily if it's an object without ID
            message_id = None
            ...
            # Call contacts using the global function
            await dial_contacts(
                contacts=contacts,
                group_id=group_id,
    ...
    async def dial_contact(self, contact_id: uuid.UUID, message_id: uuid.UUID, burn_message_token: Optional[str] = None) -> bool:
        """
        Dial a contact with a specific message.
        
        Parameters:
        - contact_id: ID of the contact to call
        - message_id: ID of the message to use for the call
        - burn_message_token: Optional token for a burn message to include in the call
